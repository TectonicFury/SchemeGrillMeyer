  ...
  (define (iter items count)
    (if (= count n)
        ...
        (let ((new-wire (make-wire)))
          (iter (cons new-wire items) (+ count 1)))))
  (iter '() 0))

(define (set-n-signals n-wires signals)
  (if (null? n-wires)
      'done
      (begin
        (set-signal! (car n-wires) (car signals))
        (set-n-signals (cdr n-wires) (cdr signals)))))

(define (set-n-probes name n-wires n)
  (define (iter count items)
    (if (null? items)
        'done
        (begin
          (probe name count (car items))
          (iter (+ 1 count) (cdr items)))))
  (iter 1 n-wires))
...
(define (probe name count wire)
  (add-action! wire (lambda ()
                      (newline)
                      (display name)(display count) (display " ")  (display (current-time the-agenda))
                      (display " New-value = ")
...
(define (inverter input output)
  (define (invert-action-procedure)
    (let ((new-value ((lambda (x)
                        (cond ((= x 0) 1)
                              ((= x 1) 0)
                              (else (error "Invalid signal")))) (get-signal input))))
